...
<template>
  <div v-if="contest_data">
    <div class="flex flex-row gap-3 items-center justify-between">
        <ReturnBtn />
        <h1 v-if="contest_data" class="text-xl font-bold">{{ t('contests.results') }} {{ contest_data.english_name }}</h1>
    </div>
    <div v-if="contest" class="mt-3">
      ...
      <p v-else class="text-red-500">
        ...
        <h2 class="w-96 text-center relative">{{ t('contests.coming') }}</h2>
    </div>
    </div>
    </p>
    </div>
  ...
  <div v-else class="p-20 w-full h-30 rounded-[18px] border-4 border-gray-400 dark:border-slate-500 border-dashed flex items-center justify-center relative">
        ...
        <h2 class="w-96 text-center relative">{{ t('contests.not_found') }}</h2>
    </div>
    </div>
</template>
