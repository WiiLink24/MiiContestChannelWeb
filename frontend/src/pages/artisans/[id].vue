...
<template>
  <div v-if="artisan_data" class="container translate-y-10">
    ...
    <div v-if="artisan_data" class="mt-3 flex sm:flex-row flex-col gap-6">
      ...
      <div class="pb-3 flex flex-col items-start justify-end">
        ...
        ><i class="fa-solid fa-download"></i> {{ $t('artisan.download') }} {{ artisan_data.name }}{{ $t('artisan.mii') }}</button>
        ...
        <p class="flex flex-col text-xl text-black">
          <span class="opacity-30 text-sm">{{ $t('artisan.number') }}</span>
          ...
          <span v-else class="text-red-500"><i class="fa-solid fa-triangle-exclamation text-yellow-300 cursor-pointer" title={{ $t('artisan.dolphin') }} onmouseover="document.getElementById('dolWarn').style.opacity = 1;" onmouseleave="document.getElementById('dolWarn').style.opacity = 0;"></i></span>
        </div>
        </p>
        <div class="flex flex-row items-center text-black">
          <p class="flex flex-col justify-end text-left text-black">
            <span class="opacity-30 text-sm">{{ $t('artisan.last') }} </span>
    ...
    <div class="mt-6 mb-3 flex flex-row gap-5 items-center justify-between">
      <h1 class="font-bold text-4xl">{{ $t('artisan.miis') }}</h1>
      <h2 v-if="artisan_data" class="opacity-60 text-right">{{ artisan_data.name }} {{ $t('artisan.submitted') }} {{ artisan_data.number_of_posts }} {{ $t('artisan.n_miis') }}</h2>
    </div>
    ...
    >
      <div class="flex flex-col items-center gap-3 text-gray-500 dark:text-slate-400">
        ...
        <h2 class="w-96 text-center relative">
          {{ artisan_data.name }} {{ $t('artisan.no_miis') }}
        </h2>
      </div>
    </div>
  </div>
  <div v-else class="container p-20 w-full h-30 translate-y-14 rounded-[18px] border-4 border-gray-400 dark:border-slate-500 border-dashed flex items-center justify-center relative">
        ...
        <h2 class="w-96 text-center relative">{{ $t('artisan.not_found') }}</h2>
    </div>
    </div>
</template>
